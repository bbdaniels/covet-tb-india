...
use "${dir}/data/constructed/sp-covet.dta" , clear
  ...
  ren cov_screen screen
    lab var screen "{&darr} {bf:Any Screening} {&darr}"
    lab var ppe "{&darr} {bf:Any Safety} {&darr}"
...
use "${dir}/data/constructed/sp-covet.dta" , clear
  ...
  betterbarci ///
    ...
    med_anti_any_3 med_anti_any_2 med_code_any_9 ///
  if city == 2 ///
    , over(case) legend(on) xlab(${pct}) xoverhang title("Mumbai") barc(red gray) bar pct vce(cluster uid)
